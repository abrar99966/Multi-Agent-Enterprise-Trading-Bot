...
- **Built today** = in the codebase, wired, tested. **Part I** (§1–§10).
- **Target-state (not built)** = the institutional north-star (Rust/Aeron/QuestDB/
  ...
  deep-dive is Part II** of this same document (the 16-section target-state
  design), which is aspirational, not current state.

If Part I and Part II ever disagree about what exists, **Part I wins** — it
describes reality; Part II describes the roadmap.
...
phased migration are **Part II** below (16 sections) — treat it as the **design
deep-dive / roadmap**, not a statement of current state.
...
end-user `USER_GUIDE.md`; target-state deep-dive = **Part II** below.

---
---

# Part II — Target-State Design (Roadmap, NOT current state)

> The sections below (1–16) are the aspirational institutional target-state design
> — Rust/Aeron/QuestDB/ClickHouse/Postgres/Qdrant/K8s. **Most of it is not built.**
> Part I above is the source of truth for what exists today; §8's status matrix
> maps each item here to its actual build state. Section numbers in this Part are
> the canonical design section numbers referenced from code comments
> (e.g. "sections 5–6" = slow/fast path design; "§7.4" = the promotion gate).
## 1. Executive Summary
